...
(function () {
  ...
  const VOICES = [
    ['正直、占いってどれも同じこと書いてあると思ってました。最初の一行が「人前では平気な顔をしていられる人ですね」で、うわ、と声が出ました。職場でも家でもそれで通してきたので。当たってるというか、見透かされた感じです。',
      '30代・女性','宿命鑑定','2026.07',5],
    ['転機の日に出てた日付、普通に何もなかったです。と思ってたら3日後に前の職場から連絡きて、まあ誤差の範囲かなと。文章自体は読みごたえありました。',
      '40代・男性','転機の日 特定鑑定','2026.06',4],
    ['母の名前で試したら総格のところで泣かれました。祖母がつけた名前なので。私自身の結果より、そっちが刺さったみたいです。',
      '20代・女性','姓名判断','2026.08',5],
    ['通勤中に引いてます。入力いらないのが地味にありがたい。',
      '30代・女性','今日引くべき一枚','2026.08',5],
    ['「確かめる質問を重ねるな」って書かれてて、それ完全に自分がやってたことでした。既読ついた?とか、今日どこ行ったの?とか。やめたら向こうから連絡くるようになったので、結果的には合ってたのかもしれません。',
      '20代・男性','あの人の本音','2026.05',4],
    ['影の章、読んでて途中で閉じました。次の日にまた開いて最後まで読みました。当たってほしくないことほど当たってて、しんどかったけど、ちゃんと書いてくれる占いは初めてでした。',
      '50代・女性','影の章','2026.04',5],
    ['文章量がとにかく多いです。有料の方は通勤の往復でもまだ読み終わらなかった。私は嬉しいですが、サッと結論だけ知りたい人には向かないかも。',
      '40代・女性','宿命鑑定','2026.07',4],
    ['相性の点数は正直ピンときませんでした。ただ「相手の速度を変えようとするな」って一文だけずっと残ってます。三年付き合ってて、ずっとそれをやってたので。',
      '30代・男性','ふたりの相性','2026.06',3],
    ['半信半疑で無料のとこだけ読むつもりが、気づいたら課金してました。まあそういう作りなんでしょうけど、後悔はしてないです。',
      '20代・女性','あの人の本音','2026.08',4]
  ];
  const starHTML = n => '<span class="stars" aria-label="5つ星のうち'+n+'">' +
    '★★★★★'.slice(0,n) + '<span class="off">' + '★★★★★'.slice(n) + '</span></span>';
  (function voices(){
    $('#voices').innerHTML = VOICES.map(([q,who,menu,date,star],i) =>
      '<article class="voice rv d'+((i%3)+1)+'"><span class="qm">&ldquo;</span><p>'+q+'</p>' +
      '<div class="who"><b>'+who+'</b>'+starHTML(star)+'<span>'+menu+'　'+date+'</span></div></article>').join('');
    watch($('#voices'));
  })();
